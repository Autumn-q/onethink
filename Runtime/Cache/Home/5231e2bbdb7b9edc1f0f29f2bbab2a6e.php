	...
	<!-- 主体 -->
        ...
        <div class="container-fluid">
            <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="row noticeList">
                <a href="<?php echo U('Article/content?id='.$row['id']);?>">
                    <div class="col-xs-2">
                        <img class="noticeImg" src="<?php echo ($row["url"]); ?>" />
                    </div>
                    <div class="col-xs-10">
                        <p class="title"><?php echo ($row["title"]); ?></p>
                        <p class="intro"><?php echo ($row["description"]); ?></p>
                        <p class="info">浏览: <?php echo ($row["view"]); ?> <span class="pull-right"><?php echo date('Y-m-d H:i:s',$row['create_time']);?></span> </p>
                    </div>
                </a>
